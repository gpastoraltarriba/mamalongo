...
import {
  LABEL,
  STATUSES,
  listenCards,               // ← lee de boards/{boardId}/cards
  saveColumnOrderForStatus, // ← guarda order en boards/{boardId}/cards
} from "../lib/BoardData";
import Header from "../components/Header";
import CardItem from "../components/CardItems"; // tu archivo se llama CardItems
import CardForm from "../components/CardForm";

export default function BoardView() {
  // Si no viene en la URL, usa un boardId demo para evitar crashear
  ...
  // Escucha las tarjetas del tablero (subcolección)
  useEffect(() => {
    const unsub = listenCards(boardId, setCards);
    return () => unsub();
  }, [boardId]);

  // Agrupa por columna y aplica orden si existe
      ...
      <div style={{ padding: 20 }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {savedAt && (
              <span style={{ color: "#16a34a" }}>Guardado ✓ {savedAt}</span>
            )}
            <button
              onClick={saveAll}
              disabled={saving}
        ...
        >
          {STATUSES.map((st, i) => (
            <div
              key={st}
              style={{ background: "#e6f4ff", borderRadius: 8, padding: 12 }}
            >
